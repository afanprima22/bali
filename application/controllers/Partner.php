...
class Partner extends MY_Controller {
	...
	public function load_data(){
		...
		$where_like['data'][] = array(
			'column' => 'partner_name,partner_telp,category_name,partner_owner,partner_address',
			...
			foreach ($query->result() as $val) {
				if ($val->category_id>0) {
					$response['data'][] = array(
						$val->category_id,
						$val->category_name,
						'<button class="btn btn-primary btn-xs" type="button" onclick="edit_data_category('.$val->category_id.')" '.$u.'><i class="glyphicon glyphicon-edit"></i></button>&nbsp;&nbsp;<button class="btn btn-danger btn-xs" type="button" onclick="delete_data_category('.$val->category_id.')" '.$d.'><i class="glyphicon glyphicon-trash"></i></button>'
					);
					$no++;	
				}
			}
	...
	public function action_data_category(){
		...
		if (strlen($id)>0) {
			//UPDATE
			$data['category_name'] = $this->input->post('i_category_name', TRUE);
			//WHERE
			$where['data'][] = array(
				'column' => 'category_id',
				'param'	 => $id
