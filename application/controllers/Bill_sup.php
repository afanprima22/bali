...
class Bill_sup extends MY_Controller {
	...
	public function load_data_detail($id){
		...
		$join['data'][] = array(
			'table' => 'notas b',
			'join'	=> 'b.nota_id=a.bill_detail_data_id and bill_detail_type = 1',
			'type'	=> 'inner'
		);
			...
			foreach ($query->result() as $val) {
				if ($val->bill_payment_id>0) {
					...
					$response['data'][] = array(
						...
						$desc,
						'<button class="btn btn-danger btn-xs" type="button" onclick="delete_data_payment('.$val->bill_payment_id.','.$val->entrusted_id.')" '.$d.'><i class="glyphicon glyphicon-trash"></i></button>'
					);
					$no++;	
				}
			}
	...
	public function action_data_detail(){
		...
		$where['data'][] = array(
			'column' => 'nota_id',
			'param'	 => $data['bill_detail_data_id']
		);
	...
	public function action_data_detail_payment(){
		...
		if ($type == 3) {
			...
			$data2['entrusted_rek'] 			= $this->input->post('i_detail_rek');
			$data2['entrusted_bank'] 			= $this->input->post('i_detail_bank');

			$insert_entrusted = $this->g_mod->insert_data_table('entrusteds', NULL, $data2);
			$entrusted_id = $insert_entrusted->output;
		}else{
			$entrusted_id = 0;
		}
		
		$data = $this->general_post_data_detail($entrusted_id);

		$insert = $this->g_mod->insert_data_table('bill_payments', NULL, $data);
		if($insert->status) {
			$response['status'] = '200';
			$response['alert'] = '1';
		} else {
	...
	public function delete_data_payment(){
		$id = $this->input->post('id');
		$entrusted = $this->input->post('entrusted');
		//WHERE
		$where['data'][] = array(
			'column' => 'entrusted_id',
			'param'	 => $entrusted
		);
		$delete1 = $this->g_mod->delete_data_table('entrusteds', $where);
		...
		$delete = $this->g_mod->delete_data_table('bill_payments', $where2);
	...
	function get_code_entrusted(){
		$bln = date('m');
		$thn = date('Y');
		$select = 'MID(entrusted_code,9,5) as id';
		$where['data'][] = array(
			'column' => 'MID(entrusted_code,1,8)',
			'param'	 => 'TU'.$thn.''.$bln.''
		);
		$order['data'][] = array(
			'column' => 'entrusted_code',
			'type'	 => 'DESC'
		);
		$limit = array(
			'start'  => 0,
		...
		$query = $this->g_mod->select($select,'entrusteds',$limit,NULL,$order,NULL,$where);
		$new_code = $this->format_kode_transaksi('TU',$query);
		return $new_code;
	}
	...
	function general_post_data_detail($entrusted_id){

		$data = array(
			'bill_id' => $this->input->post('i_id', TRUE), 
			'bill_payment_type' => $this->input->post('i_type', TRUE), 
			'bill_payment_rek' => $this->input->post('i_detail_rek', TRUE), 
			'bill_payment_bank' => $this->input->post('i_detail_bank', TRUE), 
			'entrusted_id' => $entrusted_id, 
			'bill_payment_nominal' => $this->input->post('i_nominal', TRUE), 
			'bill_payment_date' => $this->format_date_day_mid($this->input->post('i_detail_date', TRUE))
			);
