                    ...
                    <div class="box-content">
                      <div class="row">
                        <div class="col-md-6">
                          ...
                          <div class="form-group">
                            <select class="form-control select2" name="i_category" id="i_category" style="width: 80%;" required="required">
                            </select>
                            <a href="#myModal" class="btn btn-info btn-xs" data-toggle="modal"><i class="glyphicon glyphicon-plus"></i><strong>Kategori</strong></a>
                          </div>
                          <div class="form-group">
                            ...
                            <div class="box-content">-->
                              <div class="form-group">
                                <table width="100%" id="table4" class="table table-striped table-bordered bootstrap-datatable datatable responsive">
                                  <thead>
                                    <tr>
                                      <td><input type="text" class="form-control" name="i_category_id" id="i_category_id" placeholder="Auto" readonly=""></td>
                                      <td>
                                      ...
                                      <td width="10%"><button type="button" onclick="save_category()" class="btn btn-primary">Simpan satuan</button></td>
                                    </tr>
                                    <tr>
                                      <th>Id</th>
                                      <th>Nama Kategori</th>
...
<script type="text/javascript">
    ...
    function search_data_category() { 
        $('#table4').DataTable({
            ...
            "serverSide": true,
            ajax: {
              url: '<?php echo base_url();?>Partner/load_data_category/'
            },
            "columns": [
              {"name": "category_id"},
              {"name": "category_name"},
    ...
    function select_list_category() {
      ...
      function save_category(){
        $.ajax({
          type : "POST",
          url  : '<?php echo base_url();?>Partner/action_data_category/',
      ...
      function edit_data_category(id) {
        $.ajax({
          type : "GET",
          url  : '<?php echo base_url();?>Partner/load_data_where_category/',
          data : "id="+id,
          dataType : "json",
          success:function(data){
            for(var i=0; i<data.val.length;i++){
              document.getElementById("i_category_id").value               = data.val[i].category_id;
              document.getElementById("i_category_name").value             = data.val[i].category_name;
    ...
    function delete_data_category(id) {
        ...
        if(a==true){
            $.ajax({
                url: '<?php echo base_url();?>Partner/delete_data_category',
                data: 'id='+id,
                type: 'POST',
                dataType: 'json',
                success: function (data) {
